...
public class myWEKAZoo {
	public static void main(String[] args) {
		try{
			System.out.println("Hi");
			
			BufferedReader reader = new BufferedReader(new FileReader("D:/Ava/Classification2018/GenomeGoogleItem.arff"));
		...
		newTest.setClassIndex(newTest.numAttributes() - 1);
		   ...
		   //  System.out.println("0: "+newData.attribute(0));
		    ...
		    for(int i=0;i<(indices.length)/4;i++){	
		     ...
		     remove.setAttributeIndices(indices1+"last"); //Utils.arrayToString(indices)
		     remove.setInvertSelection(true);//remove not selected features
		     remove.setInputFormat(newTest);//newTest);//test1);//newTest);//test1);//kk
		     Instances instTest = Filter.useFilter(newTest, remove);//newTest,remove);//test1, remove);//newTest,remove);//kk
